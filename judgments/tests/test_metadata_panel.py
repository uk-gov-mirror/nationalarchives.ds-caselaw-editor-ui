...
class TestMetadataPanel(TestCase):
    ...
    def test_metadata_item_renders_category_names(self):
        ...
        assert '<div class="badge badge--info ">Civil</div>' in rendered
        assert '<div class="badge badge--info ">Tax</div>' in rendered
    ...
    def test_document_metadata_aside_sections_and_judges(self):
        ...
        section_headings = root.xpath('//div[contains(@class, "aside__section-heading")]/dt/text()')
    ...
    def test_metadata_panel_shows_identifiers_from_framework(self, document_type, document_exists, mock_judgment):
        ...
        self.assertContains(response, "<dt>Identifiers</dt>", html=True)
        self.assertContains(response, "<dt>Preferred</dt>", html=True)
        self.assertContains(response, "<dt>Other</dt>", html=True)
        # Preferred is highest-scoring identifier; FCLID typically outranks NCN.
    ...
    def test_metadata_panel_shows_framework_judges(self, document_type, document_exists, mock_judgment):
        ...
        judgment.metadata["judges"] = SimpleNamespace(values=["Lord Justice Underhill"])
        mock_judgment.return_value = judgment
        ...
        self.assertContains(response, '<div class="aside__section-heading">')
        self.assertContains(response, "<dt>Legacy</dt>", html=True)
